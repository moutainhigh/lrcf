...
public class JsonUtils {
	...
	/**
	 * 把String格式转换为map
	 * 就是需要将一个Map < String, Object > 这样的一个类型进行保存，后续并进行读取的功能。由于Map不能进行序列化，因此如果需要将Map整体进行保存的话，
	 * 比较不好搞，其中Map.toString()这样可以将Map转换到String但是却没有对应的反向的方法
	 *  String str4 = "{nn={en=ha}, idCard=123, phonenum=1234, map={hhaha=ni, danshi={ke=shi}}}";
	 * @param param
	 * @return
	 */
	public static Object parseMapToJSON(String param) {
		Map map = new HashMap();
		String str = "";
		String key = "";
		Object value = "";
		...
		for (int i = 0; i < charList.length; i++) {
			...
			if (c == '{') {
				if (valueBegin == true) {
					value = parseMapToJSON(param.substring(i, param.length()));
					i = param.indexOf('}', i) + 1;
					map.put(key, value);
				}
			} else if (c == '=') {
